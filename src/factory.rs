// ABOUTME: Stateless adapter factory creating channel adapters from JSON config values
...
/// Construct a channel adapter from a JSON config row
///
...
/// of maintaining a long-lived registry that must stay in sync with a database.
///
/// # Errors
///
/// Returns `MessagingError::ChannelNotConfigured` if the config is missing
...
/// Extract a required string field from a JSON config object
