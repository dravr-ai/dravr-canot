// ABOUTME: CLI entry point for the unified dravr-channels server binary
...
use dravr_channels_mcp::ServerState;
use tokio::sync::RwLock;

use dravr_channels_server::router;

/// dravr-channels-server — Unified REST API + MCP server for multi-platform messaging
#[derive(Parser)]
#[command(name = "dravr-channels-server", version, about)]
struct Cli {
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    tracing::info!(
        ...
        "Starting dravr-channels server"
    );

    match cli.transport.as_str() {
        "stdio" => {
            let server = Arc::new(dravr_channels_mcp::McpServer::new(
                Arc::clone(&state),
                dravr_channels_mcp::build_tool_registry(),
            ));
