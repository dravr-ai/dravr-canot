...
///
/// Exposes a single POST endpoint at `/mcp` that accepts JSON-RPC messages.
/// Supports both `application/json` and `text/event-stream` response formats
/// based on the client's `Accept` header (Streamable HTTP transport).
