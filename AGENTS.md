...
cargo clippy -p dravr-canot              # core library
cargo clippy -p dravr-canot-mcp          # MCP server
cargo clippy -p dravr-canot-server       # REST+MCP server
...
cargo test --workspace --features all-channels
```
